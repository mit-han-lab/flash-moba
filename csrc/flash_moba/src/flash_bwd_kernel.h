...
template<int kBlockM, bool Check_OOB, typename IdxTensor>
__forceinline__ __device__ void load_row_indices_to_smem_bwd(const int* row_indices, 
                                                          const int start_row_idx,
                                                          const int count, 
    ...
    for (int idx = threadIdx.x; idx < kBlockM; idx += blockDim.x) {
        sMobaIdx(idx) = (Check_OOB && idx >= count) ? 
                        (actual_seqlen_q + idx) : 
                        row_indices[idx + start_row_idx];
    }
    __syncthreads();  // Ensure the cache is fully populated before use
}

template<typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, typename Params>
inline __device__ void compute_moba_dq_dk_dv_1colblock(const Params &params, const int bidb, const int bidh, const int n_block) {
    ...
    auto col_indices_result = idx_gs.get_lg_col_indices(/*n_lg_block*/n_block / idx_gs.n_block_factor);
    
    const int moba_col_idx_len = col_indices_result.count;
    int *moba_row_idx_ptr = col_indices_result.indices_ptr;
    // if(threadIdx.x == 0) {
    //     printf("n_block = %d, n_factor = %d, moba_col_idx_len = %d\n", n_block, idx_gs.n_block_factor, moba_col_idx_len);
    // }
    Tensor gMobaIdx = make_tensor(make_gmem_ptr(moba_row_idx_ptr),
                            make_shape(moba_col_idx_len),
                            Stride<_1>{});
    // int m_block_max = cute::ceil_div(binfo.actual_seqlen_q, kBlockM);
    int moba_m_block_max = cute::ceil_div(moba_col_idx_len, kBlockM);

    const index_t row_offset_q = binfo.q_offset(params.q_batch_stride, params.q_row_stride, bidb) + bidh * params.q_head_stride;
    ...
    const index_t row_offset_do = binfo.q_offset(params.do_batch_stride, params.do_row_stride, bidb) + bidh * params.do_head_stride;
    ...
    const index_t row_offset_dq_accum = binfo.q_offset(params.seqlen_q_rounded * params.h * params.d_rounded, params.h * params.d_rounded, bidb)
        + (params.cu_seqlens_q == nullptr ? 0 : 128ll * bidb) * params.h * params.d_rounded + bidh * params.d_rounded
    ...
    const index_t row_offset_dpsum = (params.unpadded_lse? bidh * (params.total_q + 128 * params.b) + binfo.q_offset(params.seqlen_q_rounded, 1, bidb) + 128 * bidb: (bidb * params.h + bidh) * params.seqlen_q_rounded);

    Tensor mQ = make_gather_tensor(
        make_gmem_ptr(reinterpret_cast<Element *>(params.q_ptr) + row_offset_q),
        make_shape(moba_col_idx_len, params.d),
        make_stride(params.q_row_stride, _1{}),
        IndexedGather(moba_row_idx_ptr)
    );
    Tensor gQ = local_tile(mQ, Shape<Int<kBlockM>, Int<kHeadDim>>{}, make_coord(_, 0));
    ...
    Tensor mdO = make_gather_tensor(
        make_gmem_ptr(reinterpret_cast<Element *>(params.do_ptr) + row_offset_do),
        make_shape(moba_col_idx_len, params.d),
        make_stride(params.do_row_stride, _1{}),
        IndexedGather(moba_row_idx_ptr)
    );
    Tensor gdO = local_tile(mdO, Shape<Int<kBlockM>, Int<kHeadDim>>{}, make_coord(_, 0));

    // Tensor mO = make_gather_tensor(
    //     make_gmem_ptr(reinterpret_cast<Element *>(params.o_ptr) + row_offset_o),
    //     make_shape(moba_col_idx_len, params.d),
    //     make_stride(params.o_row_stride, _1{}),
    //     IndexedGather(moba_row_idx_ptr)
    // );
    // Tensor gO = local_tile(mO, Shape<Int<kBlockM>, Int<kHeadDim>>{}, make_coord(_, 0));
    ...
    Tensor sMobaIdx = make_tensor(
        make_smem_ptr(reinterpret_cast<int *>(smem_)),
        typename Kernel_traits::SmemLayoutMobaIdx{}
    );

    Tensor sQ = make_tensor(make_smem_ptr(reinterpret_cast<Element *>(
    ...
    //             reinterpret_cast<char *>(sdQ.data().get()) + sizeof(Element) * size(sdQ)
    //         )
    //     ),
    //     typename Kernel_traits::SmemLayoutMobaIdx{}
    // );
    ...
    // add by JXGuo: important: the accumulation dQ share the same smem as sdQ, so we can directly write back the sdQ to global memory
    //
    // PREDICATES
    //
    ...
    int moba_m_block = moba_m_block_max - 1;
    int m_row_min = !Is_causal
    ? 0
    : std::max(0, n_block * kBlockN + binfo.actual_seqlen_q - binfo.actual_seqlen_k);
    ...
    //  || gMobaIdx(moba_col_idx_len - 1) < m_row_min)
    if ((!Is_even_MN) && moba_col_idx_len <= 0) {
    ...
    // if(cute::thread0()){
    //     printf("m_block = %d, moba_col_idx_len = %d, kBlockM = %d\n", m_block, moba_col_idx_len, kBlockM);
    //     printf("moba_row_idx_ptr[0] = %d\n", moba_row_idx_ptr[0]);
    //     printf("moba_row_idx_ptr[1] = %d\n", moba_row_idx_ptr[1]);
    //     printf("moba_row_idx_ptr[2] = %d\n", moba_row_idx_ptr[2]);
    //     printf("moba_row_idx_ptr[3] = %d\n", moba_row_idx_ptr[3]);
    //     printf("moba_row_idx_ptr[4] = %d\n", moba_row_idx_ptr[4]);
    //     printf("moba_row_idx_ptr[5] = %d\n", moba_row_idx_ptr[5]);
    //     printf("moba_row_idx_ptr[6] = %d\n", moba_row_idx_ptr[6]);
    // }

    load_row_indices_to_smem_bwd<kBlockM, true>(moba_row_idx_ptr, moba_m_block * kBlockM, moba_col_idx_len - moba_m_block * kBlockM, binfo.actual_seqlen_q, sMobaIdx);

    Tensor mdQaccum = make_gather_tensor(
        make_gmem_ptr(reinterpret_cast<ElementAccum *>(params.dq_accum_ptr) + row_offset_dq_accum),
        Shape<Int<kBlockM>, Int<kHeadDim>>{},
        make_stride(params.h * params.d_rounded, _1{}),
        IndexedGather(&sMobaIdx(0))
    );
    Tensor gdQaccum = local_tile(mdQaccum, Shape<Int<kBlockM>, Int<kHeadDim>>{}, make_coord(0, 0));
    // Tensor tdQgdQaccum = gmem_thr_copy_dQaccum.partition_D(gdQaccum);
    ...
    if (Double_buffer && moba_m_block % 2 == 1) {  // Double buffer for sQ
    ...
    if (Kernel_traits::Is_V_in_regs) {
        ...
        FLASH_MOBA_NAMESPACE::cp_async_fence();
    }
    ...
    // Clear the smem tiles to account for predicated off loads
    FLASH_MOBA_NAMESPACE::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/true>(
        gmem_tiled_copy_dO, tdOgdO(_, _, _, moba_m_block), tdOsdO, tQcQ, tQpQ, moba_col_idx_len - moba_m_block * kBlockM
    );

    FLASH_MOBA_NAMESPACE::copy<Is_even_MN, Is_even_K, /*Clear_OOB_MN=*/true>(
        gmem_tiled_copy_QKV, tQgQ(_, _, _, moba_m_block), tQsQ, tQcQ, tQpQ, moba_col_idx_len - moba_m_block * kBlockM
    );
    ...
    for (int mi = 0; mi < size(lse); ++mi) {
        const int logic_row = get<0>(taccScS_row(mi)) + moba_m_block * kBlockM;
        if (logic_row < moba_col_idx_len){
            const int row = gMobaIdx(logic_row);
            lse(mi) = gLSE(row);
        } else {
            lse(mi) = INFINITY;
        }
        // const int row = logic_row < moba_act_rows ? 
    ...
    FLASH_MOBA_NAMESPACE::Dropout dropout(params.rng_state[0], params.rng_state[1], params.p_dropout_in_uint8_t,
    ...
    FLASH_MOBA_NAMESPACE::Alibi<Is_causal> alibi(alibi_slope, binfo.actual_seqlen_k, binfo.actual_seqlen_q);

    for (; moba_m_block >= 0; --moba_m_block) {
        ...
        FLASH_MOBA_NAMESPACE::gemm(acc_s, tSrQ, tSrK, tSsQ, tSsK, tiled_mma_sdp,
        ...
        Tensor scores = make_tensor(acc_s.data(), FLASH_MOBA_NAMESPACE::convert_layout_acc_rowcol(acc_s.layout()));
        // if (cute::thread(32, 0)) { print(scores); }

        // Softcapping - calculating dTanh and scaling dS later with it
        [[maybe_unused]] Tensor dtanh = make_tensor_like(scores);
        if constexpr (Is_softcap) {
            FLASH_MOBA_NAMESPACE::calculate_dtanh(scores, dtanh, params.softcap);
        }

        // Alibi
        if (Has_alibi) {
            alibi.apply_alibi(scores, n_block * kBlockN + (tidx / 32 / AtomLayoutMS) * MMA_N_SdP * 16,
                              moba_m_block * kBlockM + get<0>(taccScS_row(0)), AtomLayoutMS * 16);
        }
        ...
        if (!Is_causal) {
            if (!Is_even_MN && (n_block + 1) * kBlockN >= binfo.actual_seqlen_k) {
                FLASH_MOBA_NAMESPACE::apply_mask(scores, binfo.actual_seqlen_k,
            ...
            if (gMobaIdx(moba_m_block * kBlockM) < (n_block + 1) * kBlockN + binfo.actual_seqlen_q - binfo.actual_seqlen_k
                || (!Is_even_MN && (n_block + 1) * kBlockN >= binfo.actual_seqlen_k)) {
                FLASH_MOBA_NAMESPACE::apply_moba_mask_causal(scores, n_block * kBlockN + (tidx / 32 / AtomLayoutMS) * MMA_N_SdP * 16,
                                         binfo.actual_seqlen_k, get<0>(taccScS_row(0)),
                                         binfo.actual_seqlen_q,
                                         // binfo.actual_seqlen_k, m_block * kBlockM + (tidx / 32) % AtomLayoutMS * 16 + (tidx % 32) / 4,
                                         AtomLayoutMS * 16, sMobaIdx, moba_col_idx_len - moba_m_block * kBlockM);
            }
        } 

        // if (cute::thread(32, 0)) { print(scores); }
        // Compute the exponential value.
        FLASH_MOBA_NAMESPACE::scale_apply_exp2</*scale_max=*/false>(scores, lse, params.scale_softmax_log2);
        if constexpr (Is_dropout) {
            int warp_id = tidx / 32;
            int block_row_idx = moba_m_block * (kBlockM / 16) + warp_id % AtomLayoutMS;
        ...
        Tensor rP = !Is_dropout
            ? FLASH_MOBA_NAMESPACE::convert_type<Element>(acc_s)
            : FLASH_MOBA_NAMESPACE::convert_type_relu<Element>(acc_s);
        // Reshape rP from (MMA=4, MMA_M, MMA_N) to ((4, 2), MMA_N, MMA_N / 2)
        // if using m16n8k16 or (4, MMA_N, MMA_N) if using m16n8k8.
        Tensor tPrP = make_tensor(rP.data(), FLASH_MOBA_NAMESPACE::convert_layout_acc_Aregs<Kernel_traits::TiledMmaSdP>(rP.layout()));
        ...
        clear(acc_dp);
        // Tensor acc_dp_reshaped = make_tensor(acc_dp.data(), FLASH_MOBA_NAMESPACE::convert_layout_acc_rowcol(acc_dp.layout()));
        // #pragma unroll
        ...
        FLASH_MOBA_NAMESPACE::gemm</*A_in_regs=*/false, /*B_in_regs=*/Kernel_traits::Is_V_in_regs>(
        ...
        if (Double_buffer && moba_m_block > 0) {
            // Double buffer for sQ
            const int sQ_offset = moba_m_block % 2 == 0 ? size(sQ) : -size(sQ);
            tQsQ.data() = tQsQ.data() + sQ_offset;
            tSsQ.data() = tSsQ.data() + sQ_offset;
            // Advance gQ
            // tQgQ.data() = tQgQ.data() + (-int(kBlockM * params.q_row_stride));
            FLASH_MOBA_NAMESPACE::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, tQgQ(_, _, _, moba_m_block - 1), tQsQ, tQcQ, tQpQ);
            FLASH_MOBA_NAMESPACE::cp_async_fence();
        }

        Tensor dS_reshaped = make_tensor(dS.data(), acc_dp.layout());
        // Convert dS from fp32 to fp16
        Tensor tdSrdS = FLASH_MOBA_NAMESPACE::convert_type<Element>(dS_reshaped);
        ...
        // FLASH_MOBA_NAMESPACE::gemm_rs(acc_dv, tdVrPt, tdVrdO, tdVsdOt, tiled_mma_dkv, smem_thr_copy_QdOt);
        // Tensor tdKrdSt = make_tensor(tdSrdS.data(), tdVrPt.layout());
        // FLASH_MOBA_NAMESPACE::gemm_rs(acc_dk, tdKrdSt, tdKrQt, tdKsQt, tiled_mma_dkv, smem_thr_copy_QdOt);
        FLASH_MOBA_NAMESPACE::gemm(acc_dv, tdVrPt, tdVrdO, tdVsPt, tdVsdOt, tiled_mma_dkv,
        ...
        if (moba_m_block > 0) {
            ...
            FLASH_MOBA_NAMESPACE::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_dO, tdOgdO(_, _, _, moba_m_block - 1), tdOsdO, tQcQ, tQpQ);
            FLASH_MOBA_NAMESPACE::cp_async_fence();
        }

        FLASH_MOBA_NAMESPACE::gemm(acc_dq, tdQrdS, tdQrKt, tdQsdS, tdQsKt, tiled_mma_dq,
        ...
        if (moba_m_block > 0) {
            ...
            #pragma unroll
            for (int mi = 0; mi < size(lse); ++mi) {
                const int logic_row = get<0>(taccScS_row(mi)) + (moba_m_block - 1) * kBlockM;
                const int row = gMobaIdx(logic_row);
                lse(mi) = gLSE(row);
            }
            // #pragma unroll
        ...
        for (int i = 0; i < size(acc_dq); ++i) { 
            // int offset = tdQgdQaccum.layout()(i);
            // auto coord = tdQgdQaccum.layout().inverse(i);
            // auto coord = (0, i % size<1>(tdQgdQaccum), i / size<1>(tdQgdQaccum));
            // if (i % size<1>(tdQgdQaccum) < 6) {
            //     printf("[threadIdx.x = %d] i = %d, &tdQgdQaccum(i): %p, &tdQgdQaccum(coord): %p, acc_dq(i): %f\n", threadIdx.x, i, &tdQgdQaccum(i), &tdQgdQaccum(coord), acc_dq(i));
            // }
            // int logic_row = (i % size<1>(tdQgdQaccum)) * 8 + threadIdx.x/32;
            // // if (logic_row >= moba_col_idx_len) {
            // //     continue;
            // // }
        ...
        // if (cute::thread0()) {
        //     print(tdQgdQaccum.layout());
        //     printf("\n");
        //     print(tdQgdQaccum.shape());
        //     printf("\n");
        // }
        // if (cute::thread0()) {
        //     printf("sMobaIdx: ");
        //     print_tensor(sMobaIdx);
        //     printf("\n");
        // }


        FLASH_MOBA_NAMESPACE::gemm(acc_dk, tdKrdSt, tdKrQt, tdKsdSt, tdKsQt, tiled_mma_dkv,
                    smem_tiled_copy_PdSt, smem_tiled_copy_QdOt, smem_thr_copy_PdSt, smem_thr_copy_QdOt);
        // if (cute::thread0()) { print(acc_dk); }
        if (Double_buffer) {  // Double buffer for sQ
            tdKsQt.data() = tdKsQt.data() + (moba_m_block % 2 == 0 ? size(sQ) : -size(sQ));
        }
        if (!Double_buffer && moba_m_block > 0) {
            __syncthreads();
            // Advance gQ
        ...
        if (moba_m_block > 0) {
            const int moba_act_rows = kBlockM;
            const int start_row_idx = (moba_m_block - 1) * kBlockM;
            load_row_indices_to_smem_bwd<kBlockM, false>(moba_row_idx_ptr, start_row_idx, moba_act_rows, binfo.actual_seqlen_q, sMobaIdx);
        }
    }
    ...
    Tensor rdK = FLASH_MOBA_NAMESPACE::convert_type<Element>(acc_dk);
    Tensor rdV = FLASH_MOBA_NAMESPACE::convert_type<Element>(acc_dv);
...
template<typename Kernel_traits, bool Is_dropout, bool Is_causal, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Is_softcap, typename Params>
inline __device__ void compute_moba_dq_dk_dv_seqk_parallel(const Params &params) {
// add by JXGuo: this is used
    ...
    for (int n_block = blockIdx.x; n_block < (params.seqlen_k + Kernel_traits::kBlockN - 1) / Kernel_traits::kBlockN; n_block += gridDim.x) {
        compute_moba_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Has_alibi, Is_even_MN, Is_even_K, Is_softcap>(params, bidb, bidh, n_block);
    }
}
