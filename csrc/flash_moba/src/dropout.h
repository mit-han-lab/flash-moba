 ...
 * Adapted by Junxian Guo from https://github.com/Dao-AILab/flash-attention/blob/main/csrc/flash_attn/src/dropout.h
...
struct Dropout {
    ...
    __forceinline__ __device__ void apply_dropout(Tensor<Engine, Layout> &tensor_,
        ...
        Tensor tensor = make_tensor(tensor_.data(), FLASH_MOBA_NAMESPACE::convert_layout_acc_dropout(tensor_.layout()));
        ...
        for (int m = 0; m < size<1>(tensor); ++m, block_row_start += block_row_stride) {
            ...
            for (int n = 0; n < size<2>(tensor) / 2; ++n, ++rowcol.y) {
                ...
                uint4 random_uint4 = FLASH_MOBA_NAMESPACE::philox(seed, reinterpret_cast<unsigned long long&>(rowcol), offset);
